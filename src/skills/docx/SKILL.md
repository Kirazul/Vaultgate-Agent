...
metadata:
  author: VaultGate
  version: "1.0"
description: "Create, edit, and analyze professional Word (.docx) documents — tracked changes, comments, formatting preservation, and text extraction. Use for: (1) creating new documents, (2) modifying/editing content, (3) working with tracked changes, (4) adding comments, or any .docx task."
license: MIT
---
...
Every piece of text in the final DOCX must be readable when the document is opened in Word/LibreOffice at 100% zoom:

- **Font size**: Body text ≥ 10.5pt (CJK) / 11pt (Latin). Captions/footnotes ≥ 8pt. Never go below 8pt for any text.
- **Contrast**: Text color must visibly contrast against its background. No white-on-white, no light-gray-on-white, no dark-on-dark. Minimum luminance difference ≥ 30%.
- **No invisible text**: Never set font color equal or near-equal to the cell/page background color. Especially in tables with shaded cells — ensure text color contrasts with the shading.
- **No clipped text**: Table columns must be wide enough for their content. Use `Paragraph()` wrapping for long text — never let text overflow and become hidden.
- **Font availability**: Use standard system fonts (Times New Roman, Arial, Calibri, SimSun, Microsoft YaHei) that render on any machine. If using custom fonts, embed them or note the requirement.
- **Verification**: After generation, open the DOCX and confirm all text on every page is readable — check tables, headers, footers, and colored sections especially.
...
- **docx**: `npm install docx` (creating)
