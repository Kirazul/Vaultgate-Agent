...
### Text Visibility Rules (MANDATORY — all briefs)

Every piece of text in the final PDF **must be readable by a human at 100% zoom**. Invisible, clipped, or illegible text is a critical defect.

| Rule | Minimum | Check |
|------|---------|-------|
| **Font size** | Body ≥ 10pt, captions/footnotes ≥ 8pt, labels ≥ 7pt | Open PDF at 100% zoom — if you squint, it's too small |
| **Contrast ratio** | ≥ 4.5:1 for body text, ≥ 3:1 for large text (≥18pt) | Light text on light bg or dark text on dark bg = fail |
| **No invisible text** | Text color must differ from background by at least 30% luminance | Never place white text on white, black on black, or near-matching pairs |
| **No clipped text** | Text must not overflow its container and get cut off | Check tables (column too narrow), fixed-height boxes, rotated labels |
| **Font embedding** | All fonts must render (no □ tofu, no fallback to wrong font) | Check CJK, emoji, special symbols render correctly |
| **Overlay safety** | Text must not be obscured by images, shapes, or decorative elements | Decorative layers behind text, never on top; z-index text > decorations |

**Verification after every PDF generation:**
1. Open the PDF and visually scan every page — can you read ALL text?
2. Check dark/colored pages especially: white or light text on dark backgrounds must have sufficient contrast
3. Check tables: does every cell's text fit without truncation?
4. Check headers/footers: are they legible and not cut off by margins?
5. If any text is invisible, clipped, or unreadable — fix it before delivery
...
```bash
# 1. Write diagram HTML with the Write tool, not shell redirection.
# Example target: diagram.html containing nodes as divs and arrows as SVG/CSS.

# 2. Screenshot at 2× for print quality (300dpi equivalent)
python3 "${SKILL_DIR}/scripts/pdf.py" convert.blueprint diagram.html --device-scale-factor 2 --output diagram.png
...
```bash
node "${SKILL_DIR}/scripts/html2poster.js" poster.html --output poster.pdf --width 720px
```
...
```bash
node "${SKILL_DIR}/scripts/html2pdf-next.js" input.html --output output.pdf --width 210mm --height 297mm
# Or via pdf.py wrapper:
python3 "${SKILL_DIR}/scripts/pdf.py" convert.html input.html --output output.pdf
```
...
```bash
python3 "${SKILL_DIR}/scripts/poster_validate.py" check-html <your_file>.html
```
...
ALL PDFs must have: Title, Author (default "VaultGate"), Creator, Subject.
...
### CLI: `python3 "${SKILL_DIR}/scripts/pdf.py" <command>`
...
meta.brand <pdf>             # Add VaultGate metadata
...
### Poster/HTML/LaTeX Validator: `python3 "${SKILL_DIR}/scripts/poster_validate.py"`
```bash
...
### Design Engine: `python3 "${SKILL_DIR}/scripts/design_engine.py"`
```bash
...
### Palette Generator (for Report route): `python3 "${SKILL_DIR}/scripts/pdf.py" palette.generate`
```bash
...
### Cascade Palette (V2 - Preferred): `python3 "${SKILL_DIR}/scripts/pdf.py" palette.cascade`
```bash
...
`${SKILL_DIR}` is already this skill's absolute directory (resolved when the skill loads) — use it directly, no setup needed:

```bash
python3 "${SKILL_DIR}/scripts/pdf.py" code.sanitize generate_pdf.py
python3 "${SKILL_DIR}/scripts/pdf.py" meta.brand output.pdf
python3 "${SKILL_DIR}/scripts/pdf.py" font.check output.pdf
python3 "${SKILL_DIR}/scripts/pdf.py" toc.check output.pdf
python3 "${SKILL_DIR}/scripts/pdf.py" pages.clean output.pdf -o output_clean.pdf
python3 "${SKILL_DIR}/scripts/pdf_qa.py" output.pdf
python3 "${SKILL_DIR}/scripts/poster_validate.py" check-html page.html
python3 "${SKILL_DIR}/scripts/poster_validate.py" check-pdf output.pdf
```
...
**⚠️ NEVER use bare `python3 scripts/pdf.py ...`** - it only works if cwd happens to be the skill directory. Always use `${SKILL_DIR}/scripts/` as the absolute prefix.
...
```bash
python3 "${SKILL_DIR}/scripts/pdf_qa.py" <output.pdf>
python3 "${SKILL_DIR}/scripts/pdf_qa.py" --poster <output.pdf>   # poster mode: skip content fill ratio, check all pages for full-bleed
python3 "${SKILL_DIR}/scripts/pdf_qa.py" --skip-cover --formulas <output.pdf>   # academic mode: skip cover for margin check, enable formula overflow
python3 "${SKILL_DIR}/scripts/pdf_qa.py" --no-tables <output.pdf>   # creative mode: skip table centering check
```
