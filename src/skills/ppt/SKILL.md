...
```
python skills/ppt/ooxml/scripts/unpack.py <office_file> <output_dir>
```
...
```
[VISIBILITY] Body text ≥ 18pt, sub-body ≥ 14pt, footnotes/labels ≥ 12pt — smaller is unreadable on projection
[VISIBILITY] Contrast ratio ≥ 4.5:1 for body text against its background (including image backgrounds behind text)
[VISIBILITY] Never place text directly on a busy image without a semi-opaque mask or solid backing shape
[VISIBILITY] No invisible text: text color must visibly differ from background — no white-on-white, no dark-on-dark, no near-matching pairs
[VISIBILITY] Text must not overflow its text box or get clipped by slide boundaries
[VISIBILITY] Background images/gradients behind text must have a mask overlay (min 40% opacity solid or gradient)
[VISIBILITY] After generation, verify every slide: can you read ALL text at normal viewing distance?
```
...
2. Unpack the presentation: `python skills/ppt/ooxml/scripts/unpack.py <office_file> <output_dir>`
