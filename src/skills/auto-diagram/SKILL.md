...
## VaultGate Runtime Compatibility

- Use Excalidraw MCP calls only if the `mcp__excalidraw__...` tools are actually available in the current tool list.
- If those MCP tools are unavailable, still complete the task by producing an editable `.excalidraw` JSON file under `download/`, linked with `workspace-file:`. Optionally also produce Mermaid or SVG/HTML for quick viewing.
- Do not fail the task solely because the live Excalidraw canvas is unavailable; treat live canvas output as the preferred path and file output as the fallback path.

## Prerequisite Check

If Excalidraw MCP tools are available, verify the Excalidraw MCP server is available:

```
mcp__excalidraw__read_diagram_guide()
```

If this fails and the user specifically wants a live canvas, tell the user:
...
If MCP tools are unavailable or the user does not require a live canvas, skip this check and generate `.excalidraw` file output instead.
