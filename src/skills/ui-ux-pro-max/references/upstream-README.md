...
### Using VaultGate Marketplace (VaultGate)

Install directly in VaultGate with two commands:
...
uipro init --ai vaultgate      # VaultGate
...
**Supported:** VaultGate, Windsurf, Antigravity, Codex CLI, Continue, Gemini CLI, OpenCode, Qoder, CodeBuddy
...
> Note: If you installed via Continue, replace `.agent/skills/` with `.continue/skills/` in the commands below.
...
.agent/                     # Local dev/test for VaultGate skill
...
node dist/index.js init --ai vaultgate --offline  # Test in a temp folder
