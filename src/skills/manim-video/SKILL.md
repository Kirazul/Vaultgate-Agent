...
1. **PLAN** — Write `plan.md` with narrative arc, scene list, visual elements, color palette, on-screen text/captions
...
5. **AUDIO** (optional) — Add background music via ffmpeg if the user supplies a track. See `references/rendering.md`
6. **REVIEW** — Render preview stills, verify against plan, adjust

> Narration is on-screen text/captions in the animation — this skill does not generate spoken voiceover.
...
| `references/rendering.md` | CLI reference, quality presets, ffmpeg, background-music muxing, GIF export |
