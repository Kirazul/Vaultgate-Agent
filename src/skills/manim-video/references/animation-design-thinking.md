...
## Pacing & on-screen captions

### The "show then label" principle

Reveal the visual slightly BEFORE its caption/label appears. Seeing a circle and
THEN reading "a circle" primes the concept; the reverse makes the viewer hunt the
screen for something not there yet.

### Practical timing

```python
# Give each idea enough time to land. Reveal the element, then its caption,
# then hold so the viewer can read it.
self.play(GrowArrow(gradient_arrow), run_time=1.2)
self.play(FadeIn(Text("gradient points downhill").scale(0.6)), run_time=0.6)
self.wait(1.5)  # hold for reading
```
