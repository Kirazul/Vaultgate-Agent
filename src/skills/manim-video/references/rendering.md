...
> **Narration / voiceover is not supported.** Convey explanation through
> on-screen text, captions, and the animation itself. Time animations with
> explicit `run_time=` / `self.wait(...)` rather than syncing to spoken audio.
> The ffmpeg workflow above is only for adding a background-music track the user
> provides.
