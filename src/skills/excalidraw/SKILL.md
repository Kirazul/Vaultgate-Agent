...
- Use MCP calls such as `mcp__excalidraw__batch_create_elements` only if those tools are actually available in the current tool list.
- If Excalidraw MCP tools are not available, do not invent tool calls. Instead, generate a valid editable `.excalidraw` JSON file under `download/`, link it with `workspace-file:`, and optionally create a polished SVG/HTML/PNG companion artifact when useful.
- If the user explicitly asks for a live canvas, explain that the Excalidraw MCP/canvas bridge must be configured first; still offer the `.excalidraw` file fallback.
