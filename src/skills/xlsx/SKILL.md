...
### 7. Text Visibility (MANDATORY)

Every cell, header, label, and chart text in the final XLSX must be readable when opened in Excel/LibreOffice at 100% zoom:

- **Font size**: Data cells ≥ 10pt, headers ≥ 11pt, chart labels ≥ 9pt. Never go below 8pt for any text.
- **Contrast**: Text color must visibly contrast against cell fill. No white-on-white, no light-gray-on-light-yellow, no dark text on dark fills. When using colored header rows, ensure the font color is white or very light on dark fills, or dark on light fills.
- **No invisible text**: Never set font color equal or near-equal to the cell background/fill color. This is especially common with themed cells — verify after applying styles.
- **Column width**: Auto-fit or manually set column widths so text is not truncated. Long text cells should use wrap-text alignment.
- **Merged cells**: Text in merged cells must be fully visible — check that row height accommodates wrapped text.
- **Chart readability**: Chart axis labels, data labels, and legends must be legible. Use ≥ 9pt font, high contrast against the chart background.
- **Verification**: After generation, open the XLSX and confirm all text is visible — check styled headers, conditional-formatted cells, and chart elements especially.
...
Workbook metadata: `wb.properties.creator = "VaultGate"`
