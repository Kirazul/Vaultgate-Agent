...
function executionDiscipline(): string {
  return [
    "- STAY IN THE WORKSPACE: all file creation, edits, and scaffolding belong inside the current project/workspace root shown in Dynamic Context — use workspace-relative paths (e.g. `src/app/page.tsx`, not absolute or `../` paths that escape it). Never write to the user's home directory, system folders, or a sibling directory unless they explicitly give an absolute path and ask for it. When scaffolding a new app, create it inside the workspace (a named subfolder is fine), never outside it.",
